...
class EventBroadcaster:
    ...
    def broadcast_copilot_action(
        ...
        """
        Broadcast copilot action to all relevant surfaces (CROWN⁹ Event #10: cross_surface_sync).
        
        Ensures ≤400ms sync latency by emitting to all surface namespaces in parallel.
        
        Args:
            action: Action type (create_task, mark_done, add_to_calendar, etc.)
            result: Action result data
            workspace_id: Workspace ID
            user_id: User ID who triggered the action
            sync_target: Optional specific target (dashboard, tasks, calendar, analytics)
        ...
        try:
            ...
            event_type_map = {
                'create_task': EventType.TASK_UPDATE,
                'mark_done': EventType.TASK_UPDATE,
                'update_task': EventType.TASK_UPDATE,
                'delete_task': EventType.TASK_DELETE_SOFT,
                'add_to_calendar': EventType.MEETING_UPDATE,
                'schedule_meeting': EventType.SESSION_UPDATE_CREATED,
            }
            event_type = event_type_map.get(action, EventType.TASK_UPDATE)
            
            event = event_sequencer.create_event(
                event_type=event_type,
                event_name=f"Copilot Action: {action}",
                payload={
                    'action': action,
                    'result': result,
                    'workspace_id': workspace_id,
                ...
                trace_id=f"copilot_{action}_{int(datetime.utcnow().timestamp())}",
                workspace_id=str(workspace_id)
            )
            
            room = f"workspace_{workspace_id}"
            
            # Broadcast to all surfaces for cross-surface sync
            if self.socketio:
                surfaces = ['dashboard', 'tasks', 'calendar', 'analytics', 'copilot']
                if sync_target:
                    surfaces = [sync_target, 'dashboard']  # Always include dashboard
                ...
                logger.warning("SocketIO not initialized, cannot broadcast copilot action")
            
            # Calculate sync latency and log SLA compliance
            sync_latency_ms = (time.time() - sync_start) * 1000
            logger.info(
                f"Cross-surface sync complete: action={action}, "
                f"latency={sync_latency_ms:.0f}ms, "
                f"sla_compliant={sync_latency_ms <= 400}"
            )
            
            # Record sync latency in metrics collector (if available)
            try:
                from services.copilot_metrics_collector import copilot_metrics_collector
                copilot_metrics_collector.record_sync_latency(sync_latency_ms)
            except Exception:
                pass
            ...
            logger.error(f"Failed to broadcast copilot action: {e}")
            return None
