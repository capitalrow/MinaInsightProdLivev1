...
- **Unified Sync System** (December 2025): Consistent background synchronization across all dashboard pages via `unified-sync-init.js` included in base.html. Automatically initializes ReconciliationCycle (30-second ETag checks), WebSocket connections per page context, and BroadcastSync for cross-tab updates. Workspace ID injected via meta tag for authenticated users.
